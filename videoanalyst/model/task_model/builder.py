...
def build_cl_model(task: str,
          cfg: CfgNode,
          backbone_m: ModuleBase,
          backbone_q: ModuleBase,
          neck_m: ModuleBase,
          neck_q: ModuleBase,
          rnn: ModuleBase,
          head: ModuleBase,
    ...
    if task == "track":
        name = cfg.name
        task_module = task_modules[name](backbone_m, backbone_q, neck_m, neck_q, rnn, head, loss)
...
def build(task: str,
    ...
    if task == "track":
        name = cfg.name
        task_module = task_modules[name](backbone_m, backbone_q, neck_m, neck_q, head, loss)
        hps = task_module.get_hps()
        hps = merge_cfg_into_hps(cfg[name], hps)
        task_module.set_hps(hps)
        task_module.update_params()
