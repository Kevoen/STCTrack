...
class STCTrack(ModuleBase):
    ...
    def __init__(self, backbone_m, backbone_q, neck_m, neck_q, rnn, head, loss=None):
        super(STCTrack, self).__init__()
    ...
    def memorize(self, im_crop, fg_bg_label_map):
        # im_crop : [b, 3, 289, 289]
        # fg_bg_label_map: [1, 1, 289, 289]
        fm = self.basemodel_m(im_crop, fg_bg_label_map)
        fm = self.neck_m(fm) # [1, 768, 25, 25]->[1, 512, 25, 25]
        fm = fm.permute(1, 0, 2, 3).unsqueeze(0).contiguous()  # B, C, T, H, W (t=1)
    ...
    def train_forward(self, training_data):
        ...
        fm_p = self.rnn(fm, T, 1, T)  # b, 1, c, h, w
        fm = fm * fm_p
        fm = fm.permute(0, 2, 1, 3, 4).contiguous()
    ...
    def forward(self, *args, phase=None):
        ...
        if phase == 'train':
            ...
            fm_ext = fm
            fm_p = self.rnn(fm.permute(0, 2, 1, 3, 4).contiguous(), fm.shape[2], 1, fm.shape[2]).permute(0, 2, 1, 3, 4).contiguous()  # b, C, T, h, W
            fm = fm * fm_p

            fcos_cls_score_final, fcos_ctr_score_final, fcos_bbox_final, corr_fea, y = self.head(
            ...
            extra = dict(
                y=y,                #[1, 1024, 25, 25]
                corr_fea=corr_fea,  #[1, 256, 25, 25]
                ext_f=fm_ext,       #[1, 512, 5, 25, 25])
                rnn_f=fm_p,         #[1, 512, 5, 25, 25])
                final_f=fm,         #[1, 512, 5, 25, 25])
            )
            # output
