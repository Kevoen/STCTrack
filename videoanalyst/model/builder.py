...
from .rnn import builder as rnn_builder
...
def build(
    ...
    if task == "track":
        ...
        neck_m = neck_builder.build(task, cfg.neck)
        neck_q = neck_builder.build(task, cfg.neck)
        rnn = rnn_builder.build(task, cfg.rnn)
        head = head_builder.build(task, cfg.task_head)
        losses = loss_builder.build(task, cfg.losses)
        task_model = task_builder.build_cl_model(task, cfg.task_model, backbone_m,
                                        backbone_q, neck_m, neck_q, rnn, head, losses)
    else:
...
def get_config(task_list: List) -> Dict[str, CfgNode]:
    ...
    for task in cfg_dict:
        ...
        cfg["rnn"] = rnn_builder.get_config(task_list)[task]
