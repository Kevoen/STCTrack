...
class STCTrackTracker(PipelineBase):
    r"""
    ...
    def __init__(self, *args, **kwargs):
        super(STCTrackTracker, self).__init__(*args, **kwargs)
        self.update_params()
    ...
    def memorize(self, im: np.array, target_pos, target_sz, avg_chans):
        ...
        return fm, data.permute(1, 0, 2, 3).unsqueeze(0).contiguous()
    ...
    def select_representatives(self, cur_frame_idx):
        ...
        for idx in indexes:
            ...
            image = self._state['all_memory_frames'][idx - 1]
            if not fm.is_cuda:
    ...
    def update(self, im, state=None):
        ...
        prev_frame_feat, im_m_crop = self.memorize(self._state['last_img'],
        ...
        # stroe prev_frames
        self._state['all_memory_frames'].append(im_m_crop)

        if fidx <= self._hp_num_segments:
            features = torch.cat(self._state['all_memory_frame_feats'], dim=2)
            frames = torch.cat(self._state['all_memory_frames'], dim=2)
        else:
            features, frames = self.select_representatives(fidx)
