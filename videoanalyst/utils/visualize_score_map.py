...
SAVE_ROOT = './logs/score_map_visualization_plus/score_maps'
...
def visualize(score, score_size, crop, frame_num, name):
    ...
    print(SAVE_ROOT, '{}-{:04d}.jpg'.format(name, frame_num))


def visualize_v(score, score_size, crop, frame_num, root):
    create_dir()
    score = score.reshape(score_size, score_size)
    score = score[:, :, np.newaxis]
    score = cv2.normalize(score, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
    ...
    if not os.path.exists(root):
        os.makedirs(root)
    cv2.imwrite(os.path.join(root, '{:04d}.jpg'.format(frame_num)), final_img)
    print(SAVE_ROOT, '/{:04d}.jpg'.format(frame_num))