...
class TrackingNet(object):
    ...
    def __init__(self,
        ...
        if subset == 'test':
            ...
            self.subset_dirs = ['TRAIN_%d' % c for c in range(5)]
        else:
    ...
    def _get_cache_path_dict(self, cache_dir: str = None):
        ...
        nfs_suffix = ''
        if self.root_dir.find('/data/compute03') >= 0 and not os.path.exists('/mnt/ssddata'):
            nfs_suffix = '_data'
        elif self.root_dir.find('nfs_') >= 0 and not os.path.exists('/mnt/ssddata'):
            nfs_suffix = '_nfs'
        else:
            pass
        cache_path_dict = {
            subset: os.path.join(cache_dir, "{}.pkl".format(subset + nfs_suffix))
